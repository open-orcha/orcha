#!/usr/bin/env bash
# One-time bootstrap of the PUBLIC tap repo open-orcha/homebrew-orcha.
# Seeds README + CI only — Formula/ is owned by the release workflow, which
# pushes rendered formulae on every cli-vX.Y.Z tag (so the tap stays installable
...
gh repo view "$ORG/$TAP_REPO" >/dev/null 2>&1 || {
  echo "creating public repo $ORG/$TAP_REPO ..."
  gh repo create "$ORG/$TAP_REPO" --public \
    -d "Homebrew tap for the orcha CLI (formulae are generated by the release workflow)"
}
...
echo "  next: add TAP_GITHUB_TOKEN secret to $ORG/orcha, then tag cli-v0.2.0"
