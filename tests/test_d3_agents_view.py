...
async def test_agents_serves_and_wires_the_foundation(client):
    ...
    assert 'mountShell("agents"' in page_source("agents.html"), "agents doesn't mount the shell"
...
def test_agents_model_control_posts_ids_not_labels():
    ...
    assert 'data-model="${AgeO.esc(m.id)}"' in html, "model button doesn't carry the curated id"
...
def test_agents_model_control_filters_by_provider_runtime():
    ...
    assert 'data-runtime="${AgeO.esc(r.id)}"' in html, "provider buttons don't carry runtime ids"
    assert "modelsForRuntime(selectedRuntime)" in html, "model selector is not filtered by provider"
    assert 'id="modelSeg"' in html and 'data-runtime="${AgeO.esc(selectedRuntime)}"' in html
...
def test_agents_reasoning_effort_control_is_wired():
    ...
    assert 'data-effort="${e.id == null ? "null" : AgeO.esc(e.id)}"' in html, "effort buttons don't carry ids/null"
...
def test_agents_gate_decoupled_from_status_and_gated_on_plan_decision():
    ...
    assert "Plan awaiting your approval" in html and "Plan ${AgeO.esc(verb)}" in html, "missing the undecided/decided plan branches"
