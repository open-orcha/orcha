...
--
-- Migration number: 033 (029 = close_accepted_requests, PARKED .pending — untouched here; 030-032
-- already taken by other migrations merged onto integration/low-hanging-fruits). Pure ADDITIVE: a
-- new table, no backfill, no change to any existing row. Applied on portal boot by the R1 migration
-- runner. Reversibility: dropping this table reverts to the delivered_ts-only cursor.
