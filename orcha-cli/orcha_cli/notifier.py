...
def tick(api_base: str, cid: str, *, dry_run: bool, cooldown: float,
    ...
    for cand in scan.get("candidates", []):
        ...
        if not dry_run:
            ...
            # (--once with no reaper, tmux/unreachable) still high-water the cursor at spawn. This
            # subsumes GH#110's task-worktree withhold: a task_worktree worker is provisioned only
            # when live_workers is not None and gets added to live_workers on spawn (see above), so
            # it was never a case distinct from ephemeral_reaped — the tracked pending_ack_ts stash
            # this used to set (and reap_workers' delivered_ts emission on success) is now always a
            # no-op None, which the server treats as "no advance" and is superseded by ack-handled.
